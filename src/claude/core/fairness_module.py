...
def display_fairness_metrics():
    """
    Displays key fairness metrics relevant to AI models.
    ...
    **Key Fairness Metrics to Consider:**
    - **Demographic Parity (Statistical Parity):** Aims to ensure that the model's prediction rates are similar across different demographic groups (e.g., based on skin tone, age, gender). For instance, the proportion of individuals identified as having a bruise should be comparable across groups, assuming the underlying prevalence is similar or differences are accounted for.
    - **Equalized Odds:** Requires that the model has equal True Positive Rates (sensitivity) and equal False Positive Rates across groups. This means the model is equally good at detecting bruises when they are present, and equally good at avoiding false alarms, for all groups.
    - **Equal Opportunity:** A relaxed version of Equalized Odds, focusing only on equal True Positive Rates across groups. This ensures that if a bruise is present, all groups have an equal chance of it being detected.
    - **Predictive Rate Parity (Precision Parity):** Focuses on the Positive Predictive Value (PPV or precision) being equal across groups. This means that when the model predicts a bruise, the likelihood of it actually being a bruise is the same for all groups.
    - **Calibration:** Ensures that for a given prediction score, the probability of the outcome is the same across different groups.
    """)

def display_bias_mitigation_techniques():
    """
    Displays common bias mitigation techniques in AI.
    This function is designed to be called within a Streamlit app,
    and it will render Markdown text.
    """
    st.markdown("""
    **Common Bias Mitigation Strategies:**
    - **Pre-processing Techniques:** Applied to the training data before model training.
        - *Re-sampling:* Oversampling underrepresented groups or undersampling overrepresented groups.
    ...
    - **In-processing Techniques:** Modifications to the model training algorithm itself to incorporate fairness constraints.
        - *Adversarial Debiasing:* Training a model to make predictions while also trying to prevent an adversary from predicting the sensitive attribute from the predictions.
        - *Regularization:* Adding fairness-related penalty terms to the model's loss function.
    - **Post-processing Techniques:** Applied to the model's predictions after training.
        - *Threshold Adjustments:* Using different prediction thresholds for different demographic groups to achieve fairness goals (e.g., to equalize true positive rates).
        - *Reject Option Based Classification:* Abstaining from making a prediction for instances where the model's confidence is low or fairness might be compromised.
    - **Ongoing Auditing and Monitoring:** Regularly evaluating the model for fairness and performance disparities on new data.
    """)
